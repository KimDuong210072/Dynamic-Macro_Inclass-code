...
classdef hj_solve
    methods(Static)
        ...
        function sol = cs_hj_model_fin(par)
            ...
            amat = repmat(agrid,1,par.ylen); %a for each value of y
            
            %% Backward Induction
            u1 = zeros(alen,par.T,par.ylen); % Container for u.
            a1 = zeros(alen,par.T,par.ylen); % Container for a'.
            
            fprintf('------------Beginning Backward Induction.------------\n\n')
            % Last period: consume everything
            u1(:,par.T,:) = hj_model.utility(amat,par)'; 
            u1 = 
            a1(:,par.T,:) = 0; 
